...
static void die(const char *fmt, ...) {
  ...
  // Best-effort cleanup
  if (g_disp_pid > 0)
    kill(g_disp_pid, SIGTERM);
  if (g_eng_pid > 0)
    kill(g_eng_pid, SIGTERM);
  // Close pipes/files
  if (g_disp_in)
    fclose(g_disp_in);
  if (g_disp_out)
  ...
  if (g_disp_pid > 0) {
    usleep(100 * 1000);
    kill(g_disp_pid, SIGKILL);
  }
  if (g_eng_pid > 0) {
    usleep(100 * 1000);
    kill(g_eng_pid, SIGKILL);
  }
...
static void install_handlers(void) {
  ...
  if (sigaction(SIGPIPE, &sa, NULL) < 0)
    die("sigaction SIGPIPE: %s", strerror(errno));
  sa.sa_handler = on_sigchld;
  if (sigaction(SIGCHLD, &sa, NULL) < 0)
    die("sigaction SIGCHLD: %s", strerror(errno));
}
...
static bool read_line(FILE *in, char *buf, size_t bufsz) {
  if (!fgets(buf, (int)bufsz, in))
    return false; // EOF or error
  return true;
}

// Send exactly one line then SIGHUP to child PID, then read one-line ack.
static bool send_line_hup_expect_ack(FILE *out, pid_t child, FILE *in,
                                     const char *line, char *ackbuf,
                                     size_t acksz) {
...
static void spawn_display_if_needed(const Config *cfg) {
  ...
  int from_disp[2] = {-1, -1};
  if (pipe(to_disp) < 0)
    die("pipe to_disp: %s", strerror(errno));
  if (pipe(from_disp) < 0)
    die("pipe from_disp: %s", strerror(errno));

  pid_t pid = fork();
  if (pid < 0)
    die("fork xdisp: %s", strerror(errno));

  if (pid == 0) {
    // Child: wire stdin/stdout, close extras, exec xdisp
    if (dup2(to_disp[0], STDIN_FILENO) < 0)
      _exit(127);
    if (dup2(from_disp[1], STDOUT_FILENO) < 0)
      _exit(127);
  ...
  char ready[256];
  if (!read_line(g_disp_out, ready, sizeof(ready)))
    die("xdisp failed to signal readiness");
  // (Optional) print debug that display is ready
  // info("display ready: %s", ready);
}

// Engine is provided by engine() function in another TU; we only spawn here if
// -w/-b.
extern void engine(Board *);

static void spawn_engine_if_needed(const Config *cfg, Board *bp) {
  if (!cfg->play_white_engine && !cfg->play_black_engine)
    return; // no engine required

  int to_eng[2] = {-1, -1};
  int from_eng[2] = {-1, -1};
  if (pipe(to_eng) < 0)
    die("pipe to_eng: %s", strerror(errno));
  if (pipe(from_eng) < 0)
    die("pipe from_eng: %s", strerror(errno));
  ...
  if (pid == 0) {
    ...
    if (dup2(to_eng[0], STDIN_FILENO) < 0)
      _exit(127);
    if (dup2(from_eng[1], STDOUT_FILENO) < 0)
      _exit(127);
  ...
  // Some engine implementations may emit a ready line; the spec doesn't require
  // it. We won't block here—parent proceeds immediately.
}
...
static void parse_args(Config *cfg, int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, ":wbrvdta:i:o:")) != -1) {
    switch (opt) {
    case 'w':
      cfg->play_white_engine = true;
      break;
    case 'b':
      cfg->play_black_engine = true;
      break;
    case 'r':
      cfg->randomized_play = true;
      break;
    case 'v':
      cfg->verbose_stats = true;
      break;
    case 'd':
      cfg->no_display = true;
      break;
    case 't':
      cfg->tournament_mode = true;
      break;
    case 'a':
      cfg->avg_time = atoi(optarg);
      break;
    case 'i':
      cfg->init_file = optarg;
      break;
    case 'o':
      cfg->transcript = optarg;
      break;
    case ':':
      die("missing argument for -%c", optopt);
    default:
...
static void send_display_move(Board *bp, Player p, Move m) {
  ...
  char ack[192];
  if (!send_line_hup_expect_ack(g_disp_in, g_disp_pid, g_disp_out, line, ack,
                                sizeof ack))
...
static void load_history_if_any(Board *bp, const Config *cfg) {
  ...
  if (!f)
    die("open -i %s: %s", cfg->init_file, strerror(errno));

  while (1) {
    // read_move_from_pipe reads one move or returns 0 on EOF, validating
    // legality
...
static void notify_engine_of_opponent_move(Board *bp, Player mover,
                                           const Config *cfg, Move m) {
  if (g_eng_pid <= 0)
    return;
  ...
  char ack[192];
  if (!send_line_hup_expect_ack(g_eng_in, g_eng_pid, g_eng_out, line, ack,
                                sizeof(ack)))
...
static Move request_move_from_engine(Board *bp) {
  if (g_eng_pid <= 0)
    die("engine requested but no engine child");
  if (fputs("<\n", g_eng_in) == EOF)
    die("engine write < failed");
  if (fflush(g_eng_in) == EOF)
    die("engine flush failed");
  //    if (kill(g_eng_pid, SIGHUP) < 0) die("engine SIGHUP: %s",
  //    strerror(errno));
  // Read exactly one move line from engine and validate using the helper

  // int c;  //ming
  // while ((c = fgetc(g_eng_out)) != EOF) {   // read one character at a time
  // //ming
...
static Move request_move_from_display(Board *bp) {
  if (g_disp_pid <= 0)
    die("display requested but no display child");
  if (fputs("<\n", g_disp_in) == EOF)
    die("display write < failed");
  if (fflush(g_disp_in) == EOF)
    die("display flush failed");
  if (kill(g_disp_pid, SIGHUP) < 0)
    die("display SIGHUP: %s", strerror(errno));

  // int c;  //ming
  // while ((c = fgetc(g_disp_out)) != EOF) {   // read one character at a time
  // //ming
  //     putchar(c);                    // print it to stdout //ming
  // }
  // fprintf(stderr, "\n"); //ming
  ...
  if (m == 0)
    die("display produced EOF instead of a move");
  return m;
}
...
static void game_loop(Board *bp, const Config *cfg) {
  ...
  for (;;) {
    ...
    if (g_got_sigpipe)
      die("SIGPIPE encountered (child closed pipe)");
    ...
    fprintf(stderr,
            "[ccheck] reached echo-to-display gate. no_display=%d, "
            "came_from_display=%d\n",
    ...
    if (!cfg->no_display && !came_from_display) {
      ...
      print_move(bp, m, mem);
      fflush(mem);
      fclose(mem); /* mv now ends with '\n' */

      /* Prefix with the side, as xdisp expects */
      fprintf(stderr, "[ccheck -> xdisp] >%s", mv); // ming

      if (fprintf(g_disp_in, ">%s", mv) < 0 ||
          fflush(g_disp_in) == EOF) {
...
int ccheck(int argc, char *argv[]) {
  ...
  if (cfg.transcript) {
    ...
    if (!g_tx)
      die("open -o %s: %s", cfg.transcript, strerror(errno));
  }
